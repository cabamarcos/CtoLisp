...
sentencia_dec_funciones: IDENTIF '(' sentencia_arg ')' '{' sentencia_r '}' { sprintf (temp, "(defun %s (%s)\n\t%s\n)", $1.code, $3.code, $6.code) ;  
                                           $$.code = gen_code (temp) ; }

            | IDENTIF '(' ')' '{' sentencia_r '}' { sprintf (temp, "(defun %s ()\n\t%s\n)", $1.code, $5.code) ;  
                                           $$.code = gen_code (temp) ; }


sentencia_arg: INTEGER IDENTIF ',' sentencia_arg { sprintf (temp, "%s, %s", $2.code, $4.code) ; 
                                           $$.code = gen_code (temp) ; }
...
sentencia_r:   sentencia ';' sentencia_r { sprintf (temp, "\t%s\n\t%s", $1.code, $3.code) ;  
            ...
            | RETURN expresion ';' sentencia_r { sprintf (temp, "\t%s\n\t%s", $2.code, $4.code) ;  
                                           $$.code = gen_code (temp) ; }

            // SI SE SOLUCIONO LO OTRO BORRAR ESTE

            | RETURN expresion_control ';' { sprintf (temp, "\t%s", $2.code) ;  
                                           $$.code = gen_code (temp) ; }

            | RETURN expresion_control ';' sentencia_r { sprintf (temp, "\t%s\n\t%s", $2.code, $4.code) ;
                                             $$.code = gen_code (temp) ; }
            ;
...
sentencia_estructura_control: WHILE '(' expresion_control ')' '{' sentencia_r '}' { sprintf (temp, "(loop while %s do \n\t%s)", $3.code, $6.code) ;  
            ...
            | FOR '(' sentencia_declaraciones ';' expresion_control ';' incremento_decremento ')' '{' sentencia_r '}' { sprintf (temp, "%s\n(loop while %s do \n\t%s \n\t%s )", $3.code, $5.code, $10.code, $7.code ) ;  
                                           $$.code = gen_code (temp) ; }
...
sentencia:   PRINTF '(' STRING ',' expresion_sucesion ')'  { sprintf (temp, "%s", $5.code) ;  
                                           $$.code = gen_code (temp) ; }
...
sentencia_declaraciones: INTEGER IDENTIF '=' expresion ',' sentencia_declaraciones { sprintf (temp, "(setq %s %s) %s", $2.code, $4.code, $6.code) ; 
                                           $$.code = gen_code (temp) ; }
            ...
            | IDENTIF '=' expresion ',' sentencia_declaraciones { sprintf (temp, "(setq %s %s) %s", $1.code, $3.code, $5.code) ; 
                                           $$.code = gen_code (temp) ; }

            | IDENTIF '=' expresion { sprintf (temp, "(setq %s %s)", $1.code, $3.code) ;
                                             $$.code = gen_code (temp) ; }

            | INTEGER IDENTIF '[' NUMBER ']' ',' sentencia_declaraciones { sprintf (temp, "(setq %s (make-array %d)) %s", $2.code, $4.value, $6.code) ; 
                                           $$.code = gen_code (temp) ; }

            | INTEGER IDENTIF '[' NUMBER ']' { sprintf (temp, "(setq %s (make-array %d))", $2.code, $4.value) ;
                                                $$.code = gen_code (temp) ; }

            | IDENTIF '[' NUMBER ']' '=' NUMBER ',' sentencia_declaraciones { sprintf (temp, "(setf (aref %s %d) %d) %s", $1.code, $3.value, $6.value, $8.code) ;
                                                $$.code = gen_code (temp) ; }
...
expresion_sucesion: expresion ',' expresion_sucesion { sprintf (temp, "(print %s) %s", $1.code, $3.code) ;  
                                           $$.code = gen_code (temp) ; }
